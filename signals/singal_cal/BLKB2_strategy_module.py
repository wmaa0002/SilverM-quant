...
def check_暴力K(indicators):
    ...
    if condition:
        logger.info(f"\n=== BLK暴力K得分详情 ===")
        logger.info(f"触发条件: cond1={cond1}, cond2={cond2}, cond3={cond3}, cond4={cond4}, cond5={cond5}, cond6={cond6}, cond7={cond7}")
        logger.info(f"涨跌幅: {涨跌幅:.2f}%")
        logger.info(f"上影线比例: {上影线比例:.2%}" if K线长度 > 0 else "上影线比例: N/A")
        logger.info(f"成交量倍数: {volume/参考成交量:.2f}x" if 参考成交量 > 0 else "成交量倍数: N/A")
