...
def main():
    ...
    # 交易日检查（仅对 daily 流水线生效）
    if args.run and args.pipeline == 'daily':
        from scripts.db_utils import is_trading_day_tushare
        from datetime import date as date_cls
        today = date_cls.today()
        if not is_trading_day_tushare(today):
            logger.info(f"[{today}] 非交易日（周末/节假日），跳过数据流水线")
            return
